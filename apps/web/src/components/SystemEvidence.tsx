  ...
  return (
    <section className="panel system-evidence-panel">
      ...
      <div className="evidence-kpis">
        ...
        <article><Activity size={18} /><div><small>PRIVACY–UTILITY</small><strong>{privacy ? `SVT ε=${privacy.epsilon_parameter_per_call}` : "PENDING"}</strong><p>{privacy ? `仅共享 ${(privacy.fraction_shared * 100).toFixed(1)}% 更新 · 非端到端样本级 DP` : "等待隐私策略对照完成"}</p></div></article>
      </div>
